...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <form method="post" action="{{route('productGallery.update', ['productGallery' => $productGallery])}}" enctype="multipart/form-data">
                        @method('PATCH')
                        @csrf
                                        ...
                                        <div class="col-sm">
                                            <select class="select2 form-control mb-3 custom-select" style="width: 100%; height:36px;" name="product_id">
                                                ...
                                                @foreach($products as $product)
                                                    <option value="{{ $product->id }}" {{ $product->id == $productGallery->product_id ? 'selected' : '' }}>{{ $product->model_name }}</option>
                                                @endforeach
                                            </select>
                        ...
                        <div class="card-body">
                            <img src="{{ asset('storage/'.$productGallery->product_image) }}" style="height: 300px; width: auto" alt="" class="img-fluid mb-3">
